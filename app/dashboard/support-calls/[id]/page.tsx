import Link from "next/link";
import { notFound } from "next/navigation";
import prisma from "@/lib/prisma";

interface PageProps {
  params: Promise<{ id: string }>;
}

export default async function ViewSupportCallPage({
  params,
}: PageProps) {
  const { id } = await params;

  const supportCall = await prisma.supportCall.findUnique({
    where: {
      id: Number(id),
    },
    include: {
      practice: true,
      user: true,
      notes: true,
    },
  });

  if (!supportCall) {
    notFound();
  }

  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-100 p-8">

      <div className="mb-8 rounded-3xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-700 p-8 text-white shadow-2xl">

        <div className="flex justify-between">

          <div>

            <h1 className="text-4xl font-bold">
              📞 Support Call
            </h1>

            <p className="mt-2 text-blue-100">
              View support call information
            </p>

          </div>

          <Link
            href="/dashboard/support-calls"
            className="rounded-xl bg-white px-6 py-3 font-semibold text-indigo-700"
          >
            ← Back
          </Link>

        </div>

      </div>

      <div className="rounded-3xl bg-white p-8 shadow-2xl space-y-5">

        <Info label="Subject" value={supportCall.subject} />
        <Info label="Practice" value={supportCall.practice.practiceName} />
        <Info
          label="Assigned User"
          value={
            supportCall.user
              ? `${supportCall.user.firstName} ${supportCall.user.lastName}`
              : "Unassigned"
          }
        />
        <Info label="Priority" value={supportCall.priority} />
        <Info label="Status" value={supportCall.status} />
        <Info
          label="Description"
          value={supportCall.description ?? "-"}
        />

      </div>

      <div className="mt-8 rounded-3xl bg-white p-8 shadow-2xl">

        <h2 className="mb-6 text-2xl font-bold">
          Notes
        </h2>

        {supportCall.notes.length === 0 ? (
          <p className="text-gray-500">
            No notes available.
          </p>
        ) : (
          <div className="space-y-4">
            {supportCall.notes.map((note) => (
              <div
                key={note.id}
                className="rounded-xl border bg-gray-50 p-4"
              >
                <p>{note.note}</p>

                <p className="mt-2 text-sm text-gray-500">
                  {new Date(
                    note.createdAt
                  ).toLocaleString()}
                </p>
              </div>
            ))}
          </div>
        )}

      </div>

    </main>
  );
}

function Info({
  label,
  value,
}: {
  label: string;
  value: string;
}) {
  return (
    <div>
      <p className="text-sm font-semibold text-gray-500">
        {label}
      </p>

      <p className="mt-1 text-lg text-gray-800">
        {value}
      </p>
    </div>
  );
}